...
function loadGrid(item) {
    $("#divWithdrawApproval").jsGrid({
            ...
            loadData: function (filter) {
                return $.grep(this.data, function (item) {
                    return (!filter.EVENT_NAME || item.EVENT_NAME.toLowerCase().indexOf(filter.EVENT_NAME.toLowerCase()) > -1)
                        ...
                        && (!filter.CURRENT_APPROVER_NAME || item.CURRENT_APPROVER_NAME.toLowerCase().indexOf(filter.CURRENT_APPROVER_NAME.toLowerCase()) > -1)
        ...
        fields: [
            { name: "REQUEST_DATE", title: "Req Date", type: "text", width: 25 },
            { name: "START_DATE", title: "Start Date", type: "text", width: 25 },
            ...
            { name: "REQUESTER", title: "Requester", type: "text", width: 30 },
            { name: "CURRENT_APPROVER_NAME", title: "Current Approver", type: "text", width: 30 },
            { name: "EVENT_KEY", title: "Event Key", type: "text", width: 25 }
        ]
    });
}
...
{
    var retVal = "";
    var reqID = $('input[id$=hddUserID]').val();
    var search = {
        reqID: reqID,
        searchType: "",
        searchText: "",
    ...
    $.ajax({
        type: "POST",
        url: EVENT_SERVICE_URL + "/SelectApprovalWithdrawList",
