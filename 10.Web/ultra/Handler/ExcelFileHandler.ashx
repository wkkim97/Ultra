<%@ WebHandler Language="C#" Class="ExcelFileHandler" %>
...
public class ExcelFileHandler : IHttpHandler 
{
    ...
    private static readonly string FILES_PATH = WebSiteConfigHandler.WebServer.UploadFile.Attach.PhysicalPath;
    ...
    private static FileResponse CreateFileResponse(HttpRequest request, string fileName, string filePath, long size, string error)
    {
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (FILES_DISPOSITION)
        {
            ...
            case FilesDisposition.Absolute:
                ...
                ExcelType = context.Request.Form["ExcelType"];
                string uploadDate = DateTime.Now.ToString("yyyyMMdd_HHmmss");
                FilesPath = string.Format(@"{0}\{1}\{2}\{3}", FILES_PATH, UserID, ExcelType, uploadDate);
                break;
            default:
        ...
        if (context.Request.HttpMethod.ToUpper() == HttpMethods.GET)
        {
            ...
            else
            {
                ...
                {
                    FileResponseList.Add(CreateFileResponse(context.Request, FileName, FileName, new FileInfo(FileName).Length, String.Empty));
                }
        ...
        else if (context.Request.HttpMethod.ToUpper() == HttpMethods.POST)
        {
            ...
            for (int FileIndex = 0; FileIndex < context.Request.Files.Count; FileIndex++)
            {
                ...
                FileResponseList.Add(CreateFileResponse(context.Request, File.FileName, FileName,File.ContentLength, ErrorMessage));
            }
